...
export class Analytics {
    ...
    async retrieveDays(namespace: string, nDays: number) {
        type AnalytcisPromise = ReturnType<typeof analytics.retrieve>
        const promises:AnalytcisPromise[] = []
        ...
        const fetced = await Promise.all(promises)

        const data = fetced.sort((a,b) => {
