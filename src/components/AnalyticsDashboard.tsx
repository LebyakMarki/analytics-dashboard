...
const Badge = ({percentage}: {percentage: number}) => {
    ...
    const neutralClassname = 'bg-zinq-900/25 text-zinq-400 ring-zinq-400/25'
...
const AnalyticsDashboard = ({avgVisitorsPerDay, amntVisitorsToday, timeseriesPageviews, topCountries}:AnalyticsDashboardProps) => {
    return <div className="flex flex-col gap-6">
        <div className="grid w-full mx-auto grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <Card className="w-full mx-auto max-w-xs">
                <p className="flex gap-2.5 items-center text-tremor-default text-dark-tremor-content">
                    Visitors today
                    <Badge percentage={ (amntVisitorsToday / Number(avgVisitorsPerDay) - 1) * 100 }/>
                </p>
                <p className="text-3xl text-dark-tremor-content-strong font-semibold">
                    {amntVisitorsToday}
                </p>
        ...
        <Card>
            {timeseriesPageviews ? (
                <BarChart allowDecimals={false} showAnimation data={timeseriesPageviews.map((day) => ({
                    name: day.date,
